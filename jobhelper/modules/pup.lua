...
--[[
    The rule, the user's own, after three over-engineered attempts at
    predicting burden: back off as soon as burden STARTS, not after an
    overload. Every maneuver prints its overload chance; a nonzero print
    is the first sign of overflow, and the number itself says how hot the
    element is. Burden drains at about 0.27/s (fitted from play logs), so
    the element is rested about four seconds per point printed, plus a
    little margin -- prints 0, cast freely; prints 6, rest ~30s; prints
    23, rest ~1.5 minutes. Manual casts print too, so they feed the same
    rule. No thresholds, no prediction; the server does the measuring.
]]--

local SECONDS_PER_POINT = 4;
local BENCH_MARGIN = 8;

local benched = {};             -- ability name -> resume time (os.time)

ashita.events.register('text_in', 'jobhelper_pup_backoff', function (e)
    ...
    if (element == nil or not MANEUVERS:contains(element)) then
        return;
    end

    chance = tonumber(chance);
    if (chance > 0) then
        local name = element .. ' Maneuver';
        local rest = chance * SECONDS_PER_POINT + BENCH_MARGIN;
        benched[name] = os.time() + rest;
        util.Log(string.format('burden: %s printed %d -- resting %ds', name, chance, rest));
    end
end);

local function Benched(name, now)
    return benched[name] ~= nil and now < benched[name];
end

local function BenchWait(name, now)
    local seconds = math.max((benched[name] or now) - now, 1);
    if (seconds >= 90) then
...
-- Last logged "need" line, so the debug trail records changes rather than
-- repeating itself every four seconds.
local last_need_logged = '';
...
function M.Tick(ctx, cfg)
    ...
    if (not overloaded and ctx:Ready('Fire Maneuver') and ctx:Throttle('pup.maneuver', 4)) then
        ...
        -- back for the next stack once the timer clears. Benched elements
        -- are skipped, not just delayed, so a rested element can still go.
        local needed = util.RotationNeeds(wanted, ctx.buffs);
        if (#needed > 0) then
            local line = 'tick: need [' .. table.concat(needed, ', ') .. ']';
            if (line ~= last_need_logged) then
                util.Log(line);
                last_need_logged = line;
            end

            for _, name in ipairs(needed) do
                if (not Benched(name, ctx.now)) then
...
--[[ '/jh why': the maneuver decision, to chat and the debug log. ]]--
function M.Why(ctx, cfg)
    ...
    for _, name in ipairs(needed) do
        say(string.format('pup: need %s: %s', name,
            Benched(name, ctx.now) and ('resting ' .. BenchWait(name, ctx.now)) or 'castable'));
    end
end

--[[ '/jh burden': which elements are resting, and for how long. ]]--
function M.Debug(ctx)
    local now, any = os.time(), false;
    for name, until_t in pairs(benched) do
        if (now < until_t) then
            any = true;
            util.Message(string.format('%s resting, %s left', name, BenchWait(name, now)));
        end
    end
    if (not any) then
        util.Message('no element is resting.');
    end
end
...
function M.Draw(ctx, cfg)
    ...
    -- Elements resting off their burden, with time left.
    local held, seen = T{}, {};
    for i = 1, 3 do
        local pick = cfg.slots[i];
        if (pick ~= nil and pick > -1 and not seen[pick]) then
            ...
            if (Benched(name, ctx.now)) then
                held[#held + 1] = string.format('%s %s',
                    MANEUVERS[pick + 1], BenchWait(name, ctx.now));
            end
        end
    end
    if (#held > 0) then
        ...
        util.Tip('Resting off burden: ' .. table.concat(held, ', '));
    end
end
